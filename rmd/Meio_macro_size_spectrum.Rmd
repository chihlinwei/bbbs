...
hab <- splitBy(~Habitat+Depth.zone, data=subset(si, Wt>1e-4))
wt <- lapply(hab, FUN=function(x)x$Wt)
out <- lapply(wt, FUN=function(x)LBNbiom.method(x, binBreaks=bks))
nbss <- ldply(lapply(out, FUN=function(x)x$binVals), data.frame)
info <- ldply(strsplit(nbss$.id, split="[|]"))
...
hab <- splitBy(~Cruise+Habitat+Depth.zone, data=subset(si, Wt>5e-4))
wt <- lapply(hab, FUN=function(x)x$Wt)
out <- lapply(wt, FUN=function(x)LBNbiom.method(x, binBreaks=bks))
nbss <- ldply(lapply(out, FUN=function(x)x$binVals), data.frame)
info <- ldply(strsplit(nbss$.id, split="[|]"))
...
# Only use Wt> 1.220703e-04 (harpacticoids), 1e-4 (nematods), and 1e-2 (polychaetes)

hab_pol <- splitBy(~Cruise+Habitat+Category, data=subset(si, Category=="Polychaeta" & (Cruise=="OR1_1096"|Cruise=="OR1_1114"|Cruise=="OR1_1126")&Wt>1e-2))
hab_nem <- splitBy(~Cruise+Habitat+Category, data=subset(si, Category=="Nematoda" &  (Cruise=="OR1_1126"|Cruise=="OR1_1126")&Wt>1e-4))
hab_har <- splitBy(~Cruise+Habitat+Category, data=subset(si, Category=="Harpacticoida" & (Cruise=="OR1_1096"|Cruise=="OR1_1102"|Cruise=="OR1_1114"|Cruise=="OR1_1126")&Wt>1.220703e-04))
hab <- rbind(ldply(hab_nem), ldply(hab_har), ldply(hab_pol))
hab <- splitBy(~Habitat+Category, hab)
