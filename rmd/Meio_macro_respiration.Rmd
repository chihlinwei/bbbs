...
Meiofauna production to biomass ratio was estimated using the following equation developed by Schwinghamer et al. (1986): $P/B = 0.073*M^{-0.337}$,  where M is individual body mass [kcal]. 
...
pd <- summaryBy(productionWt+productionJ+productionC+Count+Wt~Habitat+Cruise+Station+Deployment+Tube+Subcore, data = PBresults, fun = sum, keep.names = TRUE)
pd <- cbind(p2b, pd[, -1:-6])

se <- function(x) sd(x)/length(x)^0.5
out <- summaryBy(Depth+annual.PtoB+productionWt+productionJ+productionC~Habitat+Cruise+Station, data = pd, FUN = c(mean, sd, length, se))

out <- out[, c("Habitat", "Cruise", "Station", "Depth.mean", "annual.PtoB.mean", "productionWt.mean", "productionWt.se", "productionJ.mean", "productionJ.se", "productionC.mean", "productionC.se", "Depth.length")]

names(out) <- c("Habitat", "Cruise", "Station", "Depth", "annual.PtoB", "productionWt", "productionWt.se", "productionJ", "productionJ.se", "productionC", "productionC.se", "n")
...
ggplot(data=out, 
       aes(x=Depth, y=annual.PtoB, 
           #ymin=annual.PtoB-lowerCI, ymax=annual.PtoB+upperCI,
  ...
  labs(x="Depth (m)", y=expression(Average~P/B~Ratio))+
  theme_bw() %+replace% large #%+replace% dark
```

## Generalized Least Squares (GLS) modeling

Macrofauna P/B ratio of each sample were averaged by station (i.e., average of three cores) before further statistical tests. We used Generalized Least Squares (GLS) modeling to examine the effects of habitat (canyon vs. slope), depth, and sampling time on the mean macrofauna density.
...
```{r, fig.height=5, fig.width=7}
splitBy(~Habitat, out) %>% lapply(FUN=function(x)lm(log10(productionWt)~Depth, data=x) %>% summary)

ggplot(data=out, 
       aes(x=Depth, y=log10(productionWt), 
           ymin=log10(productionWt-productionWt.se), ymax=log10(productionWt+productionWt.se),
           shape=Habitat, colour=Habitat, linetype=Habitat))+
  geom_point(size=5)+
...
```{r}
f <- gls(log10(productionWt) ~ Habitat+Depth+Cruise+Habitat:Depth+Habitat:Cruise+Depth:Cruise, data=out, method = "REML")
kable(summary(f)$tTable)
kable(anova(f))
```
...
```{r, fig.width=12, fig.height=12}
# Dianotics plot
dianostic_plot(f, y = "log10(productionWt)")
```
...
```{r}
# In canyon
f <- gls(log10(productionWt) ~ Depth+Cruise+Depth:Cruise, data=subset(out, Habitat == "Canyon"), method = "REML")
kable(summary(f)$tTable)
kable(anova(f))
# On slope
f <- gls(log10(productionWt) ~ Depth+Cruise+Depth:Cruise, data=subset(out, Habitat == "Slope"), method = "REML")
kable(summary(f)$tTable)
kable(anova(f))
# OR1 1114
f <- gls(log10(productionWt) ~ Habitat+Depth+Habitat:Depth, data=subset(out, Cruise=="OR1_1114"), method = "REML")
kable(summary(f)$tTable)
kable(anova(f))
# OR1 1126
f <- gls(log10(productionWt) ~ Habitat+Depth+Habitat:Depth, data=subset(out, Cruise=="OR1_1126"), method = "REML")
kable(summary(f)$tTable)
kable(anova(f))
```
