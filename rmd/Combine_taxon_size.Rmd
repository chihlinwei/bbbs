...
mac <- NULL
for(i in 1:15){
  d <- read_excel("../excel/GPSC_macro_size_2020.07.31.xlsx", sheet=i, col_types=col_types)
  mac <- rbind(mac, d)
}
...
ggplot(data=subset(mac_c, Category=="Polychaeta"), 
  ...
  theme_bw() %+replace% large #%+replace% dark
```
No complete polychaete specimens during cruise 1102 in canyon. 
...
mei <- NULL
for(i in 1:3){
  d <- as.data.frame(read_excel("../excel/GPSC_meio_size_2020.07.31.xlsx", sheet=i, col_types=col_types))
  mei <- rbind(mei, d)
}
