...
## All macrofauna
* Currently only OR1 1096, 1102, 1114, 1126 were included in the analysis.
...
loc <- summaryBy(Date+Longitude+Latitude+Depth~Cruise+Station+Habitat, data=as.data.frame(loc), FUN=mean, keep.names = TRUE)
loc$Date <- strftime(as.POSIXct(loc$Date, origin = "1970-01-01", tz = "Asia/Taipei"), format="%Y-%m-%d")
...
# Macrofauna productivity data

```{r}
mpr <- read.csv("../excel/prodata.csv")
```
