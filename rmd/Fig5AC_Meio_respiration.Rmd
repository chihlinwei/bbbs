...
RMresults$respirationW <- with(RMresults, daily.RtoM*Wt/(pi*(2.8/2)^2/10000))
...
(f5a <- ggplot(data=out, 
  ...
  stat_smooth(data=subset(out, Habitat=="Canyon"), method="lm", formula=y~x, alpha=0.2, size=1.5)+
...
save(f5a, file="../rds/f5a.rds")
```

Mass-specific respiration rate declined significantly with depth in the canyon but only marginally on the slope.
...
Meiofauna respirations of each sample were averaged by station (i.e., average of three cores) before further statistical tests. We used Generalized Least Squares (GLS) modeling to examine the effects of habitat (canyon vs. slope), depth, and sampling time on the mean meiofauna respirations.
...
Significant habitat and depth effect, but there are also a significant interaction between habitat and cruise.
...
kable(anova(f))
# OR1 1114
f <- gls(log10(respirationW) ~ Habitat+Depth+Habitat:Depth, data=subset(out, Cruise=="OR1_1114"), method = "REML")
kable(summary(f)$tTable)
kable(anova(f))
# OR1 1126
f <- gls(log10(respirationW) ~ Habitat+Depth+Habitat:Depth, data=subset(out, Cruise=="OR1_1126"), method = "REML")
kable(summary(f)$tTable)
kable(anova(f))
```

Significant cruise effect in canyon and significant habitat effect during OR1 1114 &  1126
