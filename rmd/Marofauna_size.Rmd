---
title: "Macrofauna Size Spectrum & Biomass for OR1 1096, 1099, 1102, 1114, 1126"
author: "Chih-Lin Wei"
date: "`r Sys.Date()`"
output: 
  html_document:
...
```{r, fig.width=10, fig.height=8}
hab <- splitBy(~Cruise+Habitat+Depth.zone, data=mac)
wt <- lapply(hab, FUN=function(x)x$Wt)
out <- lapply(wt, FUN=LBNbiom.method)
nbss <- ldply(lapply(out, FUN=function(x)x$binVals), data.frame)
info <- ldply(strsplit(nbss$.id, split="[|]"))
...
hab <- splitBy(~Cruise+Habitat+Depth.zone, data=subset(mac, Size > 1e-2))
wt <- lapply(hab, FUN=function(x)x$Wt)
out <- lapply(wt, FUN=LBNbiom.method)
nbss <- ldply(lapply(out, FUN=function(x)x$binVals), data.frame)
info <- ldply(strsplit(nbss$.id, split="[|]"))
...
wt <- wt[unlist(lapply(wt, length))>56]
out <- lapply(wt, FUN=MLE.method)
# size specutrum slope estimate by MLE
lapply(out, FUN=function(x)x[1:2])
plb <- ldply(lapply(out, FUN=function(x)x$PLB))
...
hab <- splitBy(~Habitat+Taxon, data=mac)
wt <- lapply(hab, FUN=function(x)x$Wt)
out <- lapply(wt, FUN=LBNbiom.method)
nbss <- ldply(lapply(out, FUN=function(x)x$binVals), data.frame)
info <- ldply(strsplit(nbss$.id, split="[|]"))
...
```{r, fig.width=7, fig.height=5}
library(knitr)
dat.mean <- summaryBy(Size+Depth~Region+Habitat+Cruise+Station, data=mac, FUN=c(mean, sd, length))
...
bad2 <- with(dat.mean, Cruise=="OR1_1151")
# Gaping data only
gaoping <- with(dat.mean, Region=="Gaoping")

ggplot(data=subset(dat.mean, gaoping&!(bad|bad2)), 
       aes(x=Depth.mean, y=Size.mean, 
           ymin=Size.mean-Size.sd, ymax=Size.mean+Size.sd,
           colour=Habitat, linetype=Habitat))+
  #geom_point(data=subset(dat.mean, bad2), size=5, colour="red")+
  #annotate("text", label = "OR1-1151", x = 1500, y = 0.1, size = 8, colour = "red")+
  geom_point(size=5)+
  scale_colour_manual(values=c("black","gray"))+
  geom_errorbar()+
  stat_smooth(method="lm", formula=y~x, alpha=0.2, colour="gray20")+
  scale_linetype_manual(values=c(1,2))+
  labs(x="Depth (m)", y=expression(Mean~body~size~(mg~indiv^-1)))+
  scale_y_log10()+
  theme_bw() %+replace% large

out <- dat.mean[, c("Region", "Habitat", "Cruise", "Station", "Depth.mean", "Size.length", "Size.mean", "Size.sd")]
names(out) <- c("Region", "Habitat", "Cruise", "Station", "Depth", "n", "Mean Size", "SD")
...
```{r, fig.width=7, fig.height=5}
dat.mean <- summaryBy(Size+Depth~Region+Habitat+Station, data=mac, FUN=c(mean, sd, length))
ggplot(data=subset(dat.mean, Region=="Gaoping"), 
       aes(x=Depth.mean, y=Size.mean, 
           ymin=Size.mean-Size.sd, ymax=Size.mean+Size.sd,
           colour=Habitat, linetype=Habitat))+
  geom_point(size=5)+
...
out <- dat.mean[, c("Region", "Habitat", "Station", "Depth.mean", "Size.length", "Size.mean", "Size.sd")]
names(out) <- c("Region", "Habitat", "Station", "Depth", "n", "Mean Size", "SD")
out$Depth <- round(out$Depth, 0)
out$`Mean Size` <- round(out$`Mean Size`, 2)
out$SD <- round(out$SD, 3)
kable(out, align="c")
