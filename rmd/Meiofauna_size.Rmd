---
title: "Meiofauna Size Spectrum & Biomass for OR1 1114, 1126"
author: "Chih-Lin Wei"
date: "`r Sys.Date()`"
output: 
  html_document:
...
hab <- splitBy(~Habitat+Depth.zone, data=subset(mei, (Cruise=="OR1_1114"|Cruise=="OR1_1126")&Wt>= 1e-03))
wt <- lapply(hab, FUN=function(x)x$Wt)
out <- lapply(wt, FUN=function(x)LBNbiom.method(x, binBreaks=bks))
nbss <- ldply(lapply(out, FUN=function(x)x$binVals), data.frame)
info <- ldply(strsplit(nbss$.id, split="[|]"))
...
hab <- splitBy(~Cruise+Habitat+Depth.zone, data=subset(mei, (Cruise=="OR1_1114"|Cruise=="OR1_1126")&Wt>= 1e-03))
wt <- lapply(hab, FUN=function(x)x$Wt)
out <- lapply(wt, FUN=function(x)LBNbiom.method(x, binBreaks=bks))
nbss <- ldply(lapply(out, FUN=function(x)x$binVals), data.frame)
info <- ldply(strsplit(nbss$.id, split="[|]"))
...
hab <- splitBy(~Cruise+Habitat+Category, data=subset(mei, (Cruise=="OR1_1114"|Cruise=="OR1_1126") & Category!="Others" & Wt>= 1e-03))
wt <- lapply(hab, FUN=function(x)x$Wt)
out <- lapply(wt, FUN=LBNbiom.method)
nbss <- ldply(lapply(out, FUN=function(x)x$binVals), data.frame)
info <- ldply(strsplit(nbss$.id, split="[|]"))
