...
We coverted the macrofauna individual body mass (in mg weight weight) into energy content [J] using conversion factors from Brey (2010) and then estimated macrofaunal production to biomass ratio (P/B) and annual secondary production (P) from three continuous parameters (temperature, water depth, body mass) and 17 categorical parameters (5 taxa, 7 lifestyle, 4 environment, and and state of exploitation) using the Artificial Neural Network model (ANN) developed by Brey (2012).
...
out <- out[, c("Habitat", "Cruise", "Station", "Depth.mean", "annual.PtoB", "lowerCI.mean", "upperCI.mean", "productionWt.mean", "productionWt.se", "productionJ.mean", "productionJ.se", "productionC.mean", "productionC.se", "Depth.length")]

names(out) <- c("Habitat", "Cruise", "Station", "Depth", "annual.PtoB", "lowerCI", "upperCI", "productionWt", "productionWt.se", "productionJ", "productionJ.se", "productionC", "productionC.se", "n")
...
ggplot(data=out, 
       aes(x=Depth, y=annual.PtoB, 
           #ymin=annual.PtoB-lowerCI.mean, ymax=annual.PtoB+upperCI.mean,
...
```{r, fig.height=5, fig.width=7}
splitBy(~Habitat, out) %>% lapply(FUN=function(x)lm(log10(productionWt.mean)~Depth, data=x) %>% summary)

ggplot(data=out, 
       aes(x=Depth, y=log10(productionWt.mean), 
...
```{r, fig.height=5, fig.width=7}
splitBy(~Habitat, out) %>% lapply(FUN=function(x)lm(log10(productionC.mean)~Depth, data=x) %>% summary)

ggplot(data=out, 
       aes(x=Depth, y=log10(productionC.mean), 
...
```{r, fig.height=5, fig.width=7}
splitBy(~Habitat, out) %>% lapply(FUN=function(x)lm(log10(productionJ.mean)~Depth, data=x) %>% summary)

ggplot(data=out, 
       aes(x=Depth, y=log10(productionJ.mean), 
