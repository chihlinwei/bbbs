...
ggplot()+
  stat_contour(data = mei_gr, aes(x = x, y = y, z = z, colour = ..level..))+
  #scale_colour_gradient()+
  geom_point(data=cbind(mei_md$points, fr), aes(x=MDS1, y=MDS2, fill=Habitat), pch=21, alpha=0.8, size=5, colour=gray(0, 0.2))+
  labs(colour = "Size")+
  #stat_ellipse(data=cbind(mei_md$points, fr), aes(x=MDS1, y=MDS2, fill=Habitat), type="norm")+
  annotate("text", x=-2, y=1.5, label=stress, size=5) +
  theme_bw() %+replace% large
