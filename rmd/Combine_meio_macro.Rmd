...
# Only show 1096, 1114, 1126, 1128, 1132 macrofauna size simulation because only these cruise have ok polychaete sizes
ggplot(data=subset(mac_s, Cruise=="OR1_1096"|Cruise=="OR1_1114"|Cruise=="OR1_1126"|Cruise=="OR1_1128"|Cruise=="OR1_1132"), 
       aes(x=log10(Wt), fill=Habitat))+
  geom_density(alpha=0.5)+
  scale_fill_manual(values=c("black", "gray"))+
  ...
  theme_bw() %+replace% large #%+replace% dark
```
Only cruise 1096, 1114, 1126, 1128 and 1132 can be merged

# Simulated meiofauna size distribtution
Because not all meiofauna specimens were measured for their sizes, the size distribution of nematodes (from 100 random individuals), harpacticoids, and other taxa (as many as possible) were scaled to the total abundance in three multicore tubes by re-sampling the observed size measurements (with replacement).
...
ggplot(data=subset(ldply(simu), Cruise=="OR1_1114"|Cruise=="OR1_1126"|Cruise=="OR1_1128"|Cruise=="OR1_1132"), 
       aes(x=log10(Wt), fill=Habitat))+
  geom_density(alpha=0.5)+
  scale_fill_manual(values=c("black", "gray"))+
...
mei_s <- ldply(simu)
```
Only 1114, 1126, 1128, 1132 meiofauna can be merged with macrofauna

# Merge meiofauna and macrofauna

We combine the simulated meiofauna and macrofauna size data to give the total metazoan sizes from three multicore tubes.

```{r, fig.width=10, fig.height=8}
# OR1 1114, 1126, 1128, 1132
...
si <- si[grep("1114|1126|1128|1132", si$Cruise),]
