...
ggplot(data=nbss,aes(x=log10binMid, y=log10totalBiom, colour=Class, shape=Habitat))+
  ...
  labs(x=expression(Log[10]~"mid-point of"~size~class~(mg)), y=expression(Log[10]~biomass~(mg~m^-2)), colour="Benthos")+
  theme_bw() %+replace% large #%+replace% dark
```
...
```{r, fig.width=7, fig.height=5}
ggplot(data=nbss2, aes(x=log10binMid, y=log10totalBiomNorm, colour=Class, shape=Habitat))+
  geom_point(size=5)+
  scale_colour_manual(values=c("#e41a1c", "#4daf4a"))+
  #scale_colour_viridis(discrete = TRUE)+
  scale_shape_manual(values=c(19, 1))+
